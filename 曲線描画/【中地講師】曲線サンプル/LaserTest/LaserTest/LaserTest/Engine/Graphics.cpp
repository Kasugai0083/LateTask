...
void DrawLine(const std::vector<Vec2>& list, DWORD color)
{
	// 線の頂点に使用するデータ
	struct LineVertex
	{
		LineVertex(float pos_x, float pos_y, DWORD col) :
			x(pos_x),
			y(pos_y),
			z(0.0f),
			w(1.0f),
	...
	// 範囲for(C++11 or C++14)
	for (const Vec2& pos : list)
	{
		lines.push_back(LineVertex(pos.X, pos.Y, color));
	}

	// 頂点構造の指定
	g_D3DDevice->SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE);
	g_D3DDevice->DrawPrimitiveUP(D3DPT_LINESTRIP, lines.size() - 1, &lines[0], sizeof(LineVertex));
}

void DrawRect(Vec2 pos, Size size, DWORD color)
{
	struct RectVertex
	{
		float x;
	...
	RectVertex v[4] =
	{
		...
		{ pos.X + size.Width, pos.Y, 0.0f, 1.0f, color },
		{ pos.X + size.Width, pos.Y + size.Height, 0.0f, 1.0f, color },
		{ pos.X, pos.Y + size.Height, 0.0f, 1.0f, color },
	};
...
void DrawLine(std::vector<LineDesc>& desc_list)
{
	...
	g_D3DDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, vertex_list.size() - 2, &vertex_list[0], sizeof(LineVertex));
